-- Find users with non-compliant app or firmware versions (OPTIMIZED)
-- Extended with: user_type, last measurement date, procedure count, device number
...
-- Combine photo_metadata scans into one pass
photo_stats AS (
    SELECT
        pm.user_id,
        MAX(pm.create_ts)::date AS last_measurement_date,
        COUNT(DISTINCT pm.procedure_id) AS procedure_count
...
SELECT
    ...
    ud.device_number,
    ut.user_type,
    ps.last_measurement_date,
    COALESCE(ps.procedure_count, 0) AS procedure_count,
...
CROSS JOIN config c
LEFT JOIN photo_stats ps ON ps.user_id = v.user_id
